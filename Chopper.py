...
def CordChopper(contours: Contours):
    ...
    while z > min_z:
        ...
        if not slice_exists:    
            ...
            contoursZ = ClosestContourZ(z, orig_contours) #Get the closest contour above and below the cut
            newContour = []
            for j in range(len(orig_contours[contoursZ[0]][0])): #Go over all points in closest contour
                point1 = orig_contours[contoursZ[0]][0][j]      
                point2 = ClosestPoint(point1, orig_contours[contoursZ[1]])    #Now get the closest point in second closest contour
    ...
    orig_contours.sort(key=GetZVal)  
    z = max_z - 20
    while z > min_z:
        new_contours.append([])
